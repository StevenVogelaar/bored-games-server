 ...
 * 
 * - piecePlaced -
 * 		(client) request: 	{error: false, method: 'piecePlaced', pieceID: <Number>, oX, pieceID: <Number> oY, pieceID: <Number>, dX, pieceID: <Number> dY}
 * 		(server) response:	{error: false, method: 'piecePlaced', pieceID: <Number>, oX, pieceID: <Number> oY, pieceID: <NumbeR>, dX, pieceID: <Number> dY}
 * 					// The resposne gets sent to all clients. May have 'error: true' and a message specifying the error
...
class Room extends events.EventEmitter {
	...
	constructor(roomID) {
		...
		this.logic.addListener('gameStateUpdatedHandler', this.gameStateUpdatedHandler);
	...
	gameStateUpdatedHandler(gameState, id, oX, oY, dX, dY){
